...
def similar2knn(csrcosine, k):
    ...
    for i in range(csrcosine.shape[0]):
        ...
        indicesarray = vector.argsort()[0][::-1][:k]
        knn_mat.append(list(indicesarray))
        del vector
    return knn_mat
...
def key_max(dict, orderintrain):
    # return max(dict.items(), key=lambda x: x[1])[0]  # 该max函数在两个值相同是会随机选择（应为dict.items()顺序是不定的）
    listsorted = sorted(dict.items(), key=lambda x: x[1], reverse=True)
    maxlist = []
    for t in listsorted:
        if t[1] == listsorted[0][1]:
            maxlist.append(t)
            continue
        if t[1] < listsorted[0][1]:
            break
    if len(maxlist) == 1:
        ...
        global samemaxcount
        samemaxcount += 1
        return min(maxlist, key=lambda x: orderintrain[int(x[0])])[0]
...
def inference(testinfo, traininfo, knn_mat, filename=RESULTFILE):
    ...
    result = []
    # 根据train的统计给出排名
    ageorder = [6, 1, 2, 3, 4, 5, 7]
    genorder = [3, 1, 2]
    eduorder = [4, 7, 6, 3, 2, 1, 5]
        ...
        for i, user in enumerate(testinfo):
            ...
            for j in knn_mat[i]:  # test向量中的一项
                ...
                if user_b[3] in EDU.keys():
                    EDU[user_b[3]] += 1
            user.append(key_max(AGE, ageorder))
            user.append(key_max(GEN, genorder))
            user.append(key_max(EDU, eduorder))
...
if __name__ == '__main__':
    ...
    for k in range(1, 51):
        samemaxcount = 0
        knn_mat = similar2knn(csrcosine, k)  # 通过匹配矩阵来计算匹配用户
        result = inference(test.userinfo, train.userinfo, knn_mat)  # 对用户进行推断并写入
        knnlist.append([k, samemaxcount])
        del knn_mat
    with open('knn_select_k', 'w') as f:
        for l in knnlist:
            f.write(str(l))
